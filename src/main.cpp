...
// bateria... Cada característica tem propriedades, que correspondem ao que a
// gente quer que essa característica realize (ler, escrever, notificar) O
// callback essencialmente é uma reação a um pedido do CLIENTE, como por exemplo
// "ligue o led pra mim aí" Notify notifica o cliente sempre que algo muda (a
// não ser que tenha vindo de um callback, ou seja, que o cliente PEDIU pra
// mudar) O Descriptor não viaja junto com os dados. Ele é meio que uma tag pra
// gente saber o que significa aquele dado, no notify. Confusões:
// CARACTERISTICA3->addDescriptor(new BLE2902()); e  BLEDescriptor
// *NotifyDescriptor = new BLEDescriptor(BLEUUID((uint16_t)0x2901));: Esses aí
// de cima são um tipo de descritor. Basicamente o cliente vê um botão no app na
// CARACTERISTICA3 onde ele pode ligar/desligar o fluxo que é notificado pra
// ele. Tipo, se tiver um loop que notifica um contador pra ele, ele pode
// decidir que quer parar de receber flood E o NotifyDescriptor->setValue("Esse
// aqui é o notificador."); CARACTERISTICA3->addDescriptor(NotifyDescriptor);...
// é o descritor de fato. Serve como documentação e pra facilitar a vida do dev
// do frontend. Na CARACTERISTA3, vai estar escrito "Esse aqui é o notificador",
// e o dev vai entender "ah, então é essa a característica que tá me floodando
// mensagem, né?"
...
// Ponteirões show de bola pra gente conseguir definir as propriedades das
// características e basicamente poder usar elas, ali no script
...
class MyCharacteristicCallbacks : public BLECharacteristicCallbacks {
  void onWrite(BLECharacteristic *pCharacteristic) {
    ...
    if (uuid.equals(CARACTERISTICA2)) {
      size_t length = value.length(); // Comprimento dos dados em bytes

      if (length > 0) {
        Serial.print("Dados binarios recebidos, tamanho: ");
        Serial.println(length);

        // Acessar os bytes individualmente
        // value.data() retorna um ponteiro const char*, que pode ser tratado
        // como bytes
        ...
        for (int i = 0; i < length; i++) {
          ...
          Serial.print(i);
          Serial.print("]: 0x");
          // Imprimir em formato hexadecimal para melhor visualização binária
          if (data[i] < 0x10)
            Serial.print("0");
          Serial.println(data[i], HEX);
        }

        //interpretação do byte:
        if (data[0] == 0x01) {
    ...
    if (value.length() > 0) {
      ...
      if (uuid.equals(CARACTERISTICA2)) {
        ...
        // Serial.print("O estado atual do led é: "); Serial.println(led ?
        // "LIGADO" : "DESLIGADO");
      } else if (uuid.equals(CARACTERISTICA_PERGUNTA)) {
        // LÓGICA DO INDEX

        int indice =
            atoi(value.c_str()); // Lembrando que atoi é pra converter string
                                 // pra inteiro. A gente recebe tudo em string
        Serial.print("O índice que o cliente pediu é: ");
        Serial.println(indice);

        if (indice >= 0 &&
            indice < 24) // A gente vê se é válido o número que o cliente mandou
                         // pra gente (se tá dentro das últimas 24 horas)
        {
          float temperaturaDoIndice = historicoTemperatura[indice];
          caracResposta->setValue(
              String(temperaturaDoIndice)
                  .c_str()); // Já que o nosso caracResposta lê, significa que a
                             // gente usa ele pro cliente ver a temperatura

          Serial.println("Valor atualizado.");
        }

        else
          caracResposta->setValue("Indice inválido");
      }
    }
  }
};
...
void setup() {
  ...
  carac2_Escrever = pService->createCharacteristic(
      CARACTERISTICA2, BLECharacteristic::PROPERTY_WRITE);
  ...
  carac3_Notif = pService->createCharacteristic(
      CARACTERISTICA3, BLECharacteristic::PROPERTY_NOTIFY);
  carac3_Notif->addDescriptor(
      new BLE2902()); // Esse camarada aqui é o nosso interruptor do fluxo.
                      // Então é um segundo descritor
  ...
  caracPergunta = pService->createCharacteristic(
      CARACTERISTICA_PERGUNTA, BLECharacteristic::PROPERTY_WRITE);
  caracPergunta->setCallbacks(new MyCharacteristicCallbacks());

  BLEDescriptor *QuestionDescriptor =
      new BLEDescriptor(BLEUUID((uint16_t)0x2901));
  ...
  caracResposta = pService->createCharacteristic(
      CARACTERISTICA_RESPOSTA, BLECharacteristic::PROPERTY_READ);
  BLEDescriptor *RespostaDescriptor =
      new BLEDescriptor(BLEUUID((uint16_t)0x2901));
  ...
  BLEAdvertising *pAdvertising =
      BLEDevice::getAdvertising(); // Aqui que ele começa a anunciar pra galera
...
void loop() {
  if (dispositivoConectado) {
    ...
    if (SensorTemperatura >= 40.0)
      SensorTemperatura = 25.0;
